...
"""
build_agentic_ai.py
Professional PyInstaller + cross-platform installer workflow for AgenticAI Builder
Includes:
...
- pkg/dmg on macOS
- deb/rpm on Linux
- Logging, progress bars, library checks, backups, post-build verification
"""
...
def create_windows_installer():
    ...
    if nsis_script.exists():
        ...
        if result.returncode != 0:
            ...
            log("NSIS installer created successfully.")
...
def create_linux_installers():
    ...
    if exe_path.exists():
        log("Building Debian (.deb) and RPM (.rpm) packages...")
        # Assume package folder structure exists
        deb_file = DIST_DIR / "AgenticAI_Builder.deb"
        rpm_file = DIST_DIR / "AgenticAI_Builder.rpm"
        # Simplified commands; in real use you’d generate control/spec files
        subprocess.run(["dpkg-deb", "--build", str(exe_path), str(deb_file)])
        subprocess.run(["rpmbuild", "-bb", str(exe_path)])  # Requires proper .spec file
        log(f"Linux packages created: {deb_file}, {rpm_file}")
